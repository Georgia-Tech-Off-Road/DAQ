...
class EngineDyno(DAATALayout, uiFile):
    def __init__(self, thread, is_data_collecting):
        ...
        self.gridPlotLayout = GridPlotLayout(self.SpeedAndTorqueGraphContents)
        self.gridPlotLayout.setObjectName("gridPlotLayout")
        self.SpeedAndTorqueGraphContents.setLayout(self.gridPlotLayout)
    ...
    def create_graphs(self):
        for key in self.currentKeys:
            self.dict_sensors[key]['graph_widget'] = CustomPlotWidget(key, parent=self.SpeedAndTorqueGraphContents, layout=self.gridPlotLayout, graph_width_seconds = 8)
            self.dict_sensors[key]['graph_widget'].setObjectName(key)
            self.dict_sensors[key]['graph_widget'].hide()
