function [dataArr, tt, logging_f] = txtByteReader_3_7(filename, excelname)
%% Function info
% Version configured testing day 3/7 data collection
% Andrew Hellrigel

% filename - name of file being read. Should have format "xxxx.txt"
% excelname - name of file being written to. Should have format "xxxx.xlsx"
% 
% dataArray - Array containing all data extracted from file. This is
% written to excel document
% tt = time array for plotting
% logging_f = The logging frequency of the data collected
...
dataArr = [dataArr(:,1:17), dataArr(:,22:30), dataArr(:,32)];
headers1 = ["LDS_FL(mm)", 'LDS_BL(mm)', 'Current(A)', 'Voltage(V)', 'Throttle(deg)', 'Hrs', 'Min', 'Sec', 'LDS_ECVT', 'Primary(RPM)', 'Secondary(RPM)'];
headers2 = ["FBrake(PSI)", "RBrake(PSI)", "QuatW", 'QuatX', 'QuatY', 'QuatZ', 'AccelX(m/s^2)', 'AccelY(m/s^2)', 'AccelZ(m/s^2)'];
headers3 = ["EulerX(rad)", 'EulerY(rad)', 'EulerZ(rad)', "GyroX(rad/s)", 'GyroY(rad/s)', 'GyroZ(rad/s)', 'Time(us)'];
...
%writematrix(dataArr, excelname);
%xlswrite(excelname, headers, "Sheet1", 'A1:BZ1');


%% Plotting
tt = 1:1/20:length(dataArr(:,1))/20 + 0.95;
%conversion to wheel speed:
%data(:,11)
