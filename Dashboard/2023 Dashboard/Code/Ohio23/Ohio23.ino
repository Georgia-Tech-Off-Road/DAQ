...
// Pins on teensy
  ...
  // Turn on the three LED's in a pattern to indicate startup
...
void loop() {
    ...
    if(abs(t - prev_change) >= 1000){
        ...
        displaySpeed(mph_s, gps.get_data().speed);
        prev_change = t;
    }
...
void displaySpeed(uint32_t displayText, uint32_t displayTextTwo) {
  ...
  display.println("GPS Speed: " + String(displayTextTwo));
  display.display();
}